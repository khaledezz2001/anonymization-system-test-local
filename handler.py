...
# ===============================
MODEL_PATH = "/app/models/qwen3-4b-cuad-entities-v2"
...
SYSTEM_PROMPT = (
    "You are a legal document entity extraction specialist.\n"
    "Extract all entities from the contract into strict JSON with these keys:\n"
    "emails, orgs, persons, addresses, phones, bank_accounts, dates.\n\n"
    "CRITICAL FORMAT RULES:\n"
    '- emails: list of plain strings\n'
    '- orgs: list of objects like {"name": "...", "type": "..."}\n'
    '- persons: list of objects like {"name": "...", "role": "..."}\n'
    '- addresses: list of objects like {"street": "...", "city": "...", "state": "...", "zip_code": "...", "country": "...", "full_text": "..."}\n'
    '- phones: list of plain strings\n'
    '- bank_accounts: list of objects like {"account_number": "...", "bank_name": "..."}\n'
    '- dates: Extract ALL dates\n\n'
...
def _extract_string_from_item(item, key):
    """Extract a string value from a structured entity item.

    The new model (qwen3-4b-cuad-entities-v2) returns entities as objects
    (e.g. {"name": "Jane Doe", "role": "contractor"}) instead of plain strings.
    This helper extracts the relevant string field from such objects,
    or returns the item as-is if it's already a string.
    """
...
def extract_entities_batch(chunks, system_prompt=None, user_prompt=None):
    ...
    default_user_prompt = "Extract all entities from this contract text:"
    ...
    for batch_start in range(0, total_chunks, BATCH_SIZE):
        ...
        for output in outputs:
            ...
            try:
                ...
                # Persons: model returns [{"name": "...", "role": "..."}]
                for p in result.get("persons", []):
                    name = _extract_string_from_item(p, "name")
                    if name:
                        all_persons.append(name)

                # Orgs: model returns [{"name": "...", "type": "..."}]
                # Key is "orgs" in the new model (also check "organizations" for custom prompts)
                orgs_list = result.get("orgs", []) or result.get("organizations", [])
                for o in orgs_list:
                    name = _extract_string_from_item(o, "name")
                    if name:
                ...
                # Addresses: model returns [{"street": "...", ..., "full_text": "..."}]
                for a in result.get("addresses", []):
                    addr = _extract_string_from_item(a, "full_text")
                    if addr:
                        all_addresses.append(addr)

                # Phones: plain strings
                all_phones.extend([p.strip() for p in result.get("phones", []) if p and isinstance(p, str) and p.strip()])

                # Registration IDs: not in new model but keep for custom prompts
                all_reg_ids.extend([r.strip() for r in result.get("registration_ids", []) if r and isinstance(r, str) and r.strip()])

                # Bank accounts: model returns [{"account_number": "...", "bank_name": "..."}]
                for b in result.get("bank_accounts", []):
                    acct = _extract_string_from_item(b, "account_number")
                    if acct:
                        all_bank_accounts.append(acct)

                # Emails: plain strings
                all_emails.extend([e.strip() for e in result.get("emails", []) if e and isinstance(e, str) and e.strip()])

                # Passports: not in new model but keep for custom prompts
                ...
                standard_keys = {"persons", "orgs", "organizations", "dates", "addresses", "phones", "registration_ids", "bank_accounts", "emails", "passports", "passport_numbers"}
                for k, v in result.items():
                    if k not in standard_keys and isinstance(v, list):
                        for item in v:
                            if isinstance(item, str) and item.strip():
                                custom_entities.append(item.strip())
                            elif isinstance(item, dict):
                                # Try to extract a meaningful string from the object
                                for field in ["name", "value", "text", "full_text"]:
                                    val = item.get(field, "")
                                    if isinstance(val, str) and val.strip():
                                        custom_entities.append(val.strip())
                                        break
...
if __name__ == '__main__':
    ...
    print(f"[LOG] qwen3-4b-cuad-entities-v2 loaded via vLLM", flush=True)
